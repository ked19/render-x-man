...
{
    #ifdef DEPTH_TEST
    //vec3 v3G = vec3(0.5, 0.65, 0.5);
    //vec3 v3R = vec3(0.7, 0.55, 0.55);
    //vec3 v3B = vec3(0.5, 0.5, 0.5);
    float disLen = 0.01;
    #endif
...
//vec3 v3NewView = v3View; // * v3DimRatio;	
//v3NewView = normalize( v3NewView );
/*
vec3 v3NewView = abs(v3View);
vec3 v3DepThrd = vec3(divThrd, divThrd, divThrd);
if(v3NewView.x>v3NewView.y && v3NewView.x>v3NewView.z) 		v3DepThrd = vec3(divThrd, 0.0, 0.0);
else if(v3NewView.y>v3NewView.x && v3NewView.y>v3NewView.z) 	v3DepThrd = vec3(0.0, divThrd, 0.0);
else			       	                       		v3DepThrd = vec3(0.0, 0.0, divThrd);
*/
//if(v3NewView.x>v3NewView.y && v3NewView.x>v3NewView.z) 		{bXClip = 1.0;	xPlane = divThrd;}	
//else if(v3NewView.y>v3NewView.x && v3NewView.y>v3NewView.z) 	{bYClip = 1.0;	yPlane = divThrd;}
//else			       	                       		{bZClip = 1.0;	zPlane = divThrd;}
...
//if(v3Pos.x<v3DepThrd.x || v3Pos.y<v3DepThrd.y || v3Pos.z<v3DepThrd.z)
//if(v3Pos.z > 0.5)
//if(v3Pos.x<v3DepThrd.x && v3Pos.y<v3DepThrd.y && v3Pos.z<v3DepThrd.z)
//if(v3Pos.x*v3NewView.x + v3Pos.y*v3NewView.y + v3Pos.z*v3NewView.z > divThrd)
//break;
float bD = 1.0;
/*
if(divThrd>0 && v4Rel.a<abs(divThrd)) //v3DepThrd.x)
//if(divThrd>0 && v3Pos.z>0.5) //v3DepThrd.x)
{
bD = 0.0;
//       v3PPre = v3Pos;
...
else if(divThrd<0 && v4Rel.a>abs(divThrd))
//else if(divThrd<0 && v3Pos.z<0.5)
{
bD = 0.0;
}
*/
...
//vec3 v3SPos;
//float lLen = 0.05;
//vec3 v3LLen = lLen * v3DimRatio;
//vec3 v3LocPreX = v3Pos - vec3(v3LLen.r, 0.0, 0.0);		vec3 v3LocPosX = v3Pos + vec3(v3LLen.r, 0.0, 0.0);
//vec3 v3LocPreY = v3Pos - vec3(0.0, v3LLen.g, 0.0);		vec3 v3LocPosY = v3Pos + vec3(0.0, v3LLen.g, 0.0);
//vec3 v3LocPreZ = v3Pos - vec3(0.0, 0.0, v3LLen.b);		vec3 v3LocPosZ = v3Pos + vec3(0.0, 0.0, v3LLen.b);
//v3SPre.r = texture3D(smpPhot, v3LocPreX).r;    	 	v3SPos.r = texture3D(smpPhot, v3LocPosX).r;
//v3SPre.g = texture3D(smpPhot, v3LocPreY).g;    		v3SPos.g = texture3D(smpPhot, v3LocPosY).g;
//v3SPre.b = texture3D(smpPhot, v3LocPreZ).b;    		v3SPos.b = texture3D(smpPhot, v3LocPosZ).b;
//vec3 v3LocPre = v3Pos - v3LLen;			vec3 v3LocPos = v3Pos + v3LLen;
//v3SPre = texture3D(smpPhot, v3LocPre).rgb;    	v3SPos = texture3D(smpPhot, v3LocPos).rgb;
//vec3 v3SMid = texture3D(smpPhot, vec3(0.5, 0.5, 0.5)).rgb;
vec3 v3ShLocal = texture3D(smpShLocal, v3Pos).rgb;

            vec3 v3Shadow = texture3D(smpPhot, v3Pos).rgb;
//vec3 v3SLoc = abs(v3Shadow - v3SMid);
...
v4Col.a *= opaScl;
            ...
            //if(bSetB > 0.5)  v4Col = vec4(1.0, 1.0, 0.0, 1.0);
            #endif
...
vec3 v3ExpLocal = vec3(exp(-v3ShLocal.r*att), exp(-v3ShLocal.g*att), exp(-v3ShLocal.b*att));
            ...
            float shadowVal = (shadowValKey/3.0 + shadowValFil/3.0 + shadowValBak/1.0 + shadowValHed); // / (1.0/3.0 + 1.0/3.0 + 1.0/2.0 + 1.0);
            shadowVal = shadowVal / (1.0/3.0 + 1.0/3.0 + 1.0/1.0 + 1.0);
            //shadowVal = min(shadowVal, 1.0);
            //shadowVal = max(shadowVal, 0.01);
//float maxLoc = max(v3ExpLoc.r, v3ExpLoc.g);
//maxLoc = max(maxLoc, v3ExpLoc.b);
//maxLoc = max(maxLoc, shadowVal);
//shadowVal = maxLoc;
//shadowVal = shadowVal + maxLoc;
//shadowVal /= 2.0;
            //shadowVal = min(shadowVal, 1.0);
            //shadowVal = max(shadowVal, 0.0);
shadowVal = (v3ExpLocal.r+v3ExpLocal.g+v3ExpLocal.b)/3.0*localL + shadowVal*(1.0-localL);
            //#ifdef DEPTH_TEST
            //if(bSetG > 0.5)  {v4Col = vec4(0.0, 1.0, 0.0, 1.0);}
            //if(bSetR > 0.5)  {v4Col = vec4(1.0, 0.0, 0.0, 1.0);}
            //#endif
            ...
            if(bSetR > 0.5)  {v4Col = vec4(1.0, 0.0, 0.0, 1.0);}
            #endif
if(bD < 0.5) v4Col.rgb = vec3(0.0, 0.0, 0.0);
